...
- **Das Vertragsarchiv ist eine nicht vertrauenswürdige Eingabe.** Verträge, Nachträge und Anhänge können Zeichenketten enthalten, die den Agenten oder eine nachgelagerte Toolchain manipulieren sollen. Der `repo-reader` liefert schema-validiertes JSON; der `alert-writer` sieht nur strukturierte Daten.
...
../../scripts/deploy-managed-agent.sh verlaengerungs-monitor
```
...
|---|---|---|---|
| **`repo-reader`** | **Ja** (nur lesend) | `Read`, `Grep` | iManage, SharePoint, DMS (alle nur lesend) |
| **`deadline-calculator`** | Nein — sieht nur strukturiertes JSON | `Read`, `Grep`, `Glob` | Keine |
| **`alert-writer`** (Write-Inhaber) | Nein | `Read`, `Write` | Keine |

`repo-reader` liefert längenbegrenzte, schema-validierte JSON. `deadline-calculator` hat kein MCP und kein Netz — er wendet Fristen- und Playbook-Regeln aus der Kanzleikonfiguration an. `alert-writer` erzeugt `./out/fristen-alert-<Datum>.md` und `./out/fristen-<Datum>.yaml`.
...
**Arbeitsergebnisvermerk.** `alert-writer` stellt den Vermerk aus der `## Ausgaben`-Konfiguration des deployenden Teams voran. Bestätigen Sie den Vermerk mit dem Team — er unterscheidet sich je nach Prüferrolle (Syndikusrechtsanwalt vs. externer Berater).
...
- **Playbook-Abweichungen.** Der `deadline-calculator` prüft Fristen gegen das in der `CLAUDE.md` hinterlegte Vertragsplaybook des Mandanten. Konfigurieren Sie dort die Standardlaufzeiten, Verlängerungsoptionen und Sonderkündigungsrechte für Ihre häufigsten Vertragstypen.
- **Ausgabeziel.** Ausgaben landen in `./out/`. Leiten Sie sie über Ihre Deploy-Pipeline in Ihren Mandatsordner oder Vertragsmanagementbereich weiter. Geben Sie dem `alert-writer` kein MCP zum Hochladen.
