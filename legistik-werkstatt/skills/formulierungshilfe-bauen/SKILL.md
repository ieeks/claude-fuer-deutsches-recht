---
name: formulierungshilfe-bauen
description: "Formulierungshilfe aus der Mitte des Bundestages oder Landtages aufbauen. Anwendungsfall Koalitionsfraktionen wollen Aenderung einbringen ohne neues Ressortabstimmungsverfahren und Verbandeanhoerung zu starten. Aufbau Aenderungsantrag Wortlaut knappe Begründung drei bis fuenf Saetze. Vorteil schnelle Behandlung. Risiko verkuerzte Verfassungs- und Europarechts-Prüfung. Sofort-Quercheck Verfassung Europarecht. Output Formulierungshilfen-Text bereit zur Übergabe an Koalitionsfraktionen. Abgrenzung zu referentenentwurf-bauen vollständiges Ministerialverfahren."
---

# Formulierungshilfe bauen

> Schnellweg im parlamentarischen Verfahren. Mit Vorsicht zu nutzen.

## Was ist eine Formulierungshilfe?

Ein Text, den die Ministerialverwaltung im Auftrag der Hausspitze entwirft und der dann von Abgeordneten der Koalitionsfraktionen als Änderungsantrag in das laufende parlamentarische Verfahren eingebracht wird. Formal kommt der Antrag aus der "Mitte des Bundestages" / "Mitte des Landtages", inhaltlich aus dem Ministerium.

## Vorteile

- Keine förmliche Ressortabstimmung (umgeht GGO-Verfahren)
- Keine Verbändeanhörung
- Keine Prüfung durch Normenkontrollrat
- Schnelle Beratung im Ausschuss

## Risiken

- Geringere Pruefqualitaet
- Politische Reputation kann leiden ("Schleichweg")
- Risiko verfassungs- und europarechtlicher Prüfung steigt
- Bei umfassenderen Materien rechtspolitisch unangemessen

## Wann passt eine Formulierungshilfe?

- Punktuelle Änderung
- Dringender politischer Konsens
- Bestehende parlamentarische Beratung, in die das Anliegen eingespeist werden kann
- Materie ist überschaubar

## Wann NICHT?

- Eingriff in Grundrechte erheblich
- Neue Stammgesetze
- Umfangreiche Folgenabschätzung erforderlich
- Verfassungs- oder europarechtliche Zweifelsfälle

## Aufbau einer Formulierungshilfe

### Teil 1 - Antrag

"Der Ausschuss XYZ moege beschließen:"
"Artikel ... wird wie folgt geändert: ..."

### Teil 2 - Begründung (drei bis fünf Sätze)

- Anliegen kurz
- gewählte Lösung
- Bezug zum bestehenden Verfahren

### Teil 3 - interne Anlage

- Hintergrundpapier für die Abgeordneten
- Verfassungsrechtliche Bewertung in zwei Sätzen
- Europarechtliche Bewertung in zwei Sätzen
- Erfüllungsaufwand grobe Schätzung

## Pflichtquercheck

Auch bei Formulierungshilfe muss intern geprüft werden:

- Verfassungsrecht (siehe `verfassungsmaessigkeit-quercheck`)
- Europarecht (siehe `europarechtskonformitaet`)
- Notifizierung 2015/1535 ggf. erforderlich (siehe `europarechtskonformitaet`)

## Aktuelle Rechtsprechung & Leitsätze

- BVerfG, Beschl. v. 22.01.2019 — 2 BvR 2203/18, NJW 2019, 1432 — Bestimmtheitsgebot Art. 20 Abs. 3 GG bei Gesetzesformulierungen; unbestimmter Rechtsbegriff erfordert hinreichende Konturierung durch Normtext selbst oder Gesetzessystematik
- BVerwG, Urt. v. 26.05.2022 — 8 C 11.21, NVwZ 2022, 1402 — Auslegung normtextlich unpraeziser Formulierungen anhand Gesetzessystematik und Entstehungsgeschichte; grammatikalisch eindeutiger Wortlaut schraenkt teleologische Interpretation ein
- BVerfG, Beschl. v. 29.06.2021 — 1 BvR 1516/17, BVerfGE 158, 170 Rn. 32 — Verfassungskonforme Auslegung hat Grenzen; wenn Wortlaut eindeutig gegen verfassungskonformes Ergebnis spricht, ist Norm nichtig statt uminterpretierbar

## Zentrale Normen (Paragrafenkette)

§§ 1 ff. HdR (Handbuch der Rechtsfoermlichkeit Bundesregierung, Normenklarheit) — Art. 20 Abs. 3 GG (Bestimmtheitsgebot) — Art. 103 Abs. 2 GG (Bestimmtheitsgebot Strafnormen) — § 42 VwVfG (Bestimmtheit Verwaltungsakt als Auslegungsmassstab fuer Gesetzesnormen)

## Kommentarliteratur

- Schneider, Gesetzgebung, 3. Aufl. 2002, §§ 3-5 Rn. 1 ff. (Sprachregeln, Bestimmtheitsgrundsatz, Normenklarheit)
- Maurer/Waldhoff, Allgemeines Verwaltungsrecht, 20. Aufl. 2020, § 4 Rn. 20 ff. (Bestimmtheit, unbestimmte Rechtsbegriffe)

## Ausgabe

Datei "Formulierungshilfe_XYZ.md" mit Antrag, Begründung und Hintergrundpapier.

## Anschluss

`verfassungsmaessigkeit-quercheck`, `europarechtskonformitaet`.
