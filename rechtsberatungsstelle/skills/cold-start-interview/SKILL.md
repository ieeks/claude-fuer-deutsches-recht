...
description: >
  Einmalige Ersteinrichtung der Beratungsstelle. Führt den anleitenden Volljuristen
  durch ein strukturiertes Interview zur Konfiguration des Plugins: Beratungsstellentyp,
  Rechtsgrundlage, Fachbereiche, Aufsichtsmodell, Verschwiegenheitshinweise,
  Pädagogikhaltung. Ergebnis wird in CLAUDE.md gespeichert. Aufrufen bei Neugründung
...
4. Empfehle als nächsten Schritt: `/rechtsberatungsstelle:build-guide` für jeden Fachbereich.
...
Bevor Studierende `/ramp` starten und Mandate aufnehmen, muss der anleitende Volljurist die Beratungsstelle konfigurieren. Diese Konfiguration steuert:
...
**Zielgruppe: ausschließlich der anleitende Volljurist.** Studierende starten mit `/rechtsberatungsstelle:ramp`.
...
- `/rechtsberatungsstelle:build-guide` für jeden konfigurierten Fachbereich
- `/rechtsberatungsstelle:ramp` – Testlauf aus Studierenden-Perspektive
...
- **Semesterwechsel nicht organisiert:** Ohne klare Übergaberegeln fallen Mandate zwischen den Semestern durch. `/rechtsberatungsstelle:semester-handoff` muss im Ablaufplan verankert sein.
