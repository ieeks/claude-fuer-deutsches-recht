...
description: >
  Semesterabschluss-Übergabe — das Gegenstück zu `/ramp`. Erstellt fallbezogene
  Übergabenotizen und eine Kohorten-Gesamtübersicht, damit die abgehende
  Kohorte die laufenden Mandate unter Wahrung des Mandatsgeheimnisses sauber
  an die nächste übergibt. Liest Fristendatei, Mandantenkommunikationslog und
  Fallhistorie. Lädt, wenn der Supervisor oder abgehende Studierende den
...
Jedes Semester verliert die Rechtsberatungsstelle ihre gesamte studentische Belegschaft und baut sie neu auf. `/ramp` löst die eine Hälfte des Problems — Onboarding der neuen Kohorte. Diese Skill löst die andere Hälfte: Sie offboardet die abgehende Kohorte, indem sie Übergabenotizen erstellt, die alle für den Fortgang notwendigen Informationen zu jedem laufenden Mandat enthalten.
...
Nach Supervisoren-Prüfung liegen die Übergabenotizen unter `handoffs/[semester]/[fall-id].md`. Der/die Nachfolger/-in liest sie im `/ramp`-Durchlauf zu Semesterbeginn — `/ramp` soll die Notizen für die zugewiesenen Fälle surfacen.
...
- **Fristenübergabe vergessen:** Die gefährlichste Lücke. Alle aktiven Fristen müssen in der Notiz erscheinen; Nachfolger/-in muss sie in `/deadlines` als eigene Fristen neu eintragen.
