...
name: customize
description: >
  Kanzleiprofil, Allowlist, Registry-Watchlist und Aktualisierungseinstellungen anpassen.
  Einsetzen, wenn der Nutzer einzelne Profil-Abschnitte ändern möchte, neue Registries
  hinzufügen, Allowlist-Einträge aktualisieren oder die Update-Benachrichtigungseinstellungen
  ändern möchte — ohne das vollständige cold-start-interview zu wiederholen.
argument-hint: "[--profil | --allowlist | --registries | --updates | --tom | Abschnittsname]"
language: de
triggers:
...
# /customize — Kanzleiprofil und Einstellungen anpassen
...
- Angabe, welcher Abschnitt angepasst werden soll:
  ...
  - `--allowlist` — Registries, Publisher, Konnektoren, Lizenzen hinzufügen/entfernen
...
Konfiguration aus dem Config-Pfad lesen. Wenn die Datei nicht existiert oder noch Platzhalter enthält: den Nutzer auf `/kanzlei-builder-hub:cold-start-interview` hinweisen.
...
### Schritt 5: Allowlist anpassen (bei --allowlist)

Bei Allowlist-Änderungen:
...
3. Zur `allowlist.yaml` und zum CLAUDE.md-Watchlist-Abschnitt hinzufügen
...
3. Zu `publishers` in `allowlist.yaml` hinzufügen

**Modus-Wechsel (restrictive ↔ permissive):**
- Bei Wechsel nach `restrictive`: Alle vorhandenen installierten Skills sind weiterhin nutzbar, aber neue Installationen erfordern Allowlist-Eintrag.
...
```
/kanzlei-builder-hub:customize --profil
...
✅ Gespeichert. Der related-skills-surfacer wird ab sofort auch Datenschutz-Skills berücksichtigen.
```

### Beispiel 2: TOM-Status aktualisieren

```
/kanzlei-builder-hub:customize --tom
...
- **Allowlist-Modus-Wechsel ohne Überlegung:** Permissiver Modus ist bequemer, aber weniger sicher. Für Kanzleibetrieb mit Mandantendaten `restrictive` bevorzugen.
...
- Das vollständige Interview ersetzen. Bei Ersteinrichtung `/kanzlei-builder-hub:cold-start-interview` ausführen.
