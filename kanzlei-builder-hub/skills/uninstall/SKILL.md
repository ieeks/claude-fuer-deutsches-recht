...
Den vollständigen Deinstallations-, Deaktivierungs- und Reaktivierungsworkflow lädt dieser Skill aus dem `skill-manager`-Referenz-Skill — dieser muss vor substanzieller Arbeit geladen sein.
...
1. **Nur Community-Skills deinstallieren, die über diesen Hub installiert wurden.** `~/.claude/plugins/config/kanzlei-builder-hub/install-log.yaml` und die CLAUDE.md-Installationsübersicht prüfen. Ist der Skill dort nicht verzeichnet: Ablehnen und erklären.
...
4. **Deinstallation protokollieren.** An `install-log.yaml` mit Aktion `uninstall` und Zeitstempel anhängen, damit das Prüfprotokoll vollständig bleibt.

### Schritt 2: skill-manager laden

Den vollständigen Deinstallationsworkflow aus dem `skill-manager`-Referenz-Skill laden und ausführen.
...
Vollständige Workflow-Schritte gemäß `skill-manager`:

1. Verifizierung der Community-Installation aus `install-log.yaml`
...
Protokolleintrag:    install-log.yaml aktualisiert (action: uninstall)
...
1. `install-log.yaml` gelesen — `miet-kuendigung-analyse` als Community-Skill gefunden, letzter Status `install`.
