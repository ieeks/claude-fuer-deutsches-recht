# Zulässigkeitsliste (Allowlist-Konfiguration)

Der Installer unterstützt eine Zulässigkeitsliste unter:

```
~/.claude/plugins/config/claude-fuer-deutsches-recht/kanzlei-builder-hub/allowlist.yaml
```
...
Wenn `allowlist.yaml` nicht vorhanden ist, behandelt der Installer die Umgebung als `permissiv` mit einer leeren Zulässigkeitsliste — alles ist „nicht auf der Liste", sodass jede Installation eine Warnung ausgibt und der Benutzer diese explizit akzeptieren muss, bevor etwas geschrieben wird.
